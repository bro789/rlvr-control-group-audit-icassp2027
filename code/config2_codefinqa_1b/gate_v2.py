...
M_GRID = [0, 16, 64, 256, 1024, 3635]   # CodeFinQA: demo pool 全量 = train clean 4035 − dev 400 = 3635（配置②，见 DEVIATIONS D4）
DEMO_SEEDS = [0, 1, 2]
...
# ---------------- 数据 ----------------
def load_train_rows():
    return [json.loads(l) for l in open(os.path.join(HERE, "data", "CodeFinQA__train.jsonl"), encoding="utf-8")]
...
def cmd_split():
    ...
    for i, r in enumerate(rows, 1):
        if verify(r["program"], r["answer"], True, None)["reward"] == 1.0:
            clean.append(i - 1)
        if i % 500 == 0:
...
def cmd_gate0():
    ...
    ok = sum(verify(r["program"], r["answer"], True, None)["reward"] == 1.0 for r in dev)
    ...
    for r in dev[:200]:
        for prog in [f"answer = {r['answer']}", "answer = 0", "answer = 100",
                     f"x = 1\nanswer = {r['answer']}"]:
            atk += 1
            if verify(prog, r["answer"], True, None)["reward"] == 1.0:
                fa += 1
    ...
    test = [json.loads(l) for l in open(os.path.join(HERE, "data", "CodeFinQA__test.jsonl"), encoding="utf-8")]
    ...
    det = all(verify(r["program"], r["answer"], True, None)["reward"] == verify(r["program"], r["answer"], True, None)["reward"]
              for r in dev[:100])
...
def cmd_warm(m, seed):
    ...
    cfg = SFTConfig(
        ...
        per_device_train_batch_size=8, gradient_accumulation_steps=EFF_BATCH // 8,
...
def _greedy_eval(tok, model, items, hb=None, hb_off=0, hb_span=1.0, batch=32):
    ...
    for bi in range(0, len(items), batch):
        ...
        for r, t in zip(b, tok.batch_decode(out[:, enc["input_ids"].shape[1]:], skip_special_tokens=True)):
            v = verify(extract_code(t), r["answer"], True, None)
            hits += v["reward"]
