...
def out_dir(branch, m, seed, tseed=None):
    ...
    **训练**种子」。原实现用同一个 --seed 同时决定演示子集与训练 RNG，无法在固定
    ...
    if tseed is not None and tseed != seed:
        base += f"_t{tseed}"
    # 超参扫描的产物必须另存，否则会覆盖协议默认设置下的主实验结果。
    # 默认值（beta=0.04, temp=1.0）不加后缀，保证既有路径不变。
    _b, _t = os.environ.get("GRPO_BETA"), os.environ.get("GRPO_TEMP")
    if (_b and float(_b) != 0.04) or (_t and float(_t) != 1.0):
        base += f"_b{_b or '0.04'}t{_t or '1.0'}"
...
def cmd_rssft(m, seed, seconds, round_qs=64, max_vcalls=0, tseed=None, budget_mode="both"):
    ...
    while time.time() - t0 < seconds and vcalls < max_vcalls:
        ...
        base = AutoModelForCausalLM.from_pretrained(cur, dtype=torch.bfloat16)
...
def cmd_grpo(m, seed, seconds, tseed=None):
    ...
    # 超参敏感性扫描（审稿关切：负结论是否只在某一组超参下成立）。
    # 环境变量覆盖，缺省即协议冻结的默认值，故不影响已完成的主实验。
    _beta = float(os.environ.get("GRPO_BETA", "0.04"))
    _temp = float(os.environ.get("GRPO_TEMP", "1.0"))
    cfg = GRPOConfig(output_dir=od, seed=SEED + TS, bf16=True, report_to="none",
                     max_steps=100000, learning_rate=1e-5, beta=_beta,
                     num_generations=GEN_K, per_device_train_batch_size=16,
                     gradient_accumulation_steps=4, generation_batch_size=16,
                     max_completion_length=MAX_COMP, temperature=_temp, top_p=0.95,
                     logging_steps=1, save_strategy="steps", save_steps=10 ** 9,
