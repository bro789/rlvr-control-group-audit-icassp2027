...
PROMPT_TMPL = """You are a financial analyst. The context below is an excerpt from a financial report, containing narrative text and tables. Answer the question by writing a short Python program: read the relevant numbers directly from the context, write them as numeric literals in the program, compute, and assign the final numeric result to a variable named `answer`. If the question asks for a percentage, `answer` should be the percentage value (e.g. 12.5 for 12.5%). Output ONLY the Python code, no explanations.

Example 1:
Question: What was the percentage change in revenue from 2018 to 2019?
Context: ... total revenue was $550 million in 2019 compared to $500 million in 2018 ...
Code:
revenue_2019 = 550
revenue_2018 = 500
...
Example 2:
Question: What was the total operating expenses for 2018 and 2019 combined?
Context: ... operating expenses were $120.5 million in 2019 and $88.3 million in 2018 ...
Code:
opex_2019 = 120.5
opex_2018 = 88.3
answer = opex_2019 + opex_2018
...
def load_eval_items(n_items):
    rows = [json.loads(l) for l in open(os.path.join(HERE, "data", "CodeFinQA__test.jsonl"), encoding="utf-8")]
    clean = [r for r in rows if verify(r["program"], r["answer"], True, None)["reward"] == 1.0]
...
def main(model_id, n_items=200, k=64):
    ...
    for bi in range(0, len(items), BATCH):
        ...
        for j, r in enumerate(batch_items):
            gold = r["answer"]
            g_res = verify(extract_code(greedy[j]), gold, True, None)
