...
def run_variant(name, store, items, workers=12):
    ...
    def check(args):
        ...
        v = verify(p, r["answer"], True, str(r.get("context")) if r.get("context") is not None else None)
...
def main(m, seed, variant, dev_n):
    ...
    train_rows = [json.loads(l) for l in
                  open(os.path.join(HERE, "data", "CodeTAT-QA__train.jsonl"), encoding="utf-8")]
